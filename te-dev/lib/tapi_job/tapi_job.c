...
/** Format string for autogenerated job names */
#define TAPI_JOB_NAME_FMT "tapi_job.%u"
/** Maximum serial number of a job with autogenerated name */
#define TAPI_JOB_AUTOGENERATED_MAX UINT_MAX
             ...
             int ref_count, tapi_job_channel_t *channel)
{
    ...
    channel->silent_pass = job != NULL ? job->silent_pass : rpcs->silent_pass;
...
/* See description in tapi_job_internal.h */
const char *
tapi_job_get_ta(const tapi_job_t *job)
{
    if (job == NULL)
        TEST_FAIL("Job is NULL");
    ...
    return job->factory->backend.ta;
}

/* See description in tapi_job_internal.h */
const char *
tapi_job_get_name(const tapi_job_t *job)
{
    if (job == NULL)
        TEST_FAIL("Job is NULL");
...
/* Memory allocated for the name should be freed after usage */
static te_errno
generate_job_name(char **name)
{
    static unsigned int serial_number = 1;
    char *generated_name;

    if (serial_number >= TAPI_JOB_AUTOGENERATED_MAX)
    {
        ERROR("Too many jobs with autogenerated names");
        return TE_RC(TE_TAPI, TE_ETOOMANY);
    }

    generated_name = te_string_fmt(TAPI_JOB_NAME_FMT, serial_number++);
    if (generated_name == NULL)
    {
        ERROR("Not enough memory for job's name");
                ...
                const char **env, tapi_job_t **job)
{
    ...
    {
        case TAPI_JOB_FACTORY_RPC:
            return tapi_job_create_named(factory, NULL, spawner, program, argv,
                                         env, job);
        ...
        case TAPI_JOB_FACTORY_CFG:
        {
            ...
            rc = generate_job_name(&name);
            if (rc != 0)
                return rc;

            rc = tapi_job_create_named(factory, name, spawner, program, argv,
                                       env, job);
            free(name);
            return rc;
        }
                      ...
                      const char **argv, const char **env, tapi_job_t **job)
{
    ...
    switch (factory->type)
    {
        case TAPI_JOB_FACTORY_RPC:
            ...
            tapi_job->silent_pass = factory->backend.rpcs->silent_pass;
            break;

        case TAPI_JOB_FACTORY_CFG:
            if (name == NULL)
            {
                ERROR("Name must be specified for jobs created by CFG factory");
                free(tapi_job);
                return TE_RC(TE_TAPI, TE_EINVAL);
            }

            tapi_job_set_name(tapi_job, name);
            break;
...
tapi_job_dealloc_channels(tapi_job_channel_set_t channels)
{
    ...
    silent_pass = rpcs->silent_pass;
    rpcs->silent_pass = channels[0]->silent_pass;
                       ...
                       tapi_job_channel_t **filter)
{
    ...
    if (filter != NULL)
    {
        ...
        /*
         * After rpc_job_attach_filter() the rpcs->silent_pass may be set
         * to default value. To mute rpc logs related to a filter,
         * the filter has to inherit the silent_pass value of channel
         * via rpcs->silent_pass.
         */
        rpcs->silent_pass = channels[0]->silent_pass;
...
tapi_job_poll(const tapi_job_channel_set_t wait_set, int timeout_ms)
{
    ...
    silent_pass = rpcs->silent_pass;
    rpcs->silent_pass = wait_set[0]->silent_pass;
                                              ...
                                              tarpc_job_buffer *buffer))
{
    ...
    silent_pass = rpcs->silent_pass;
    rpcs->silent_pass = filters[0]->silent_pass;

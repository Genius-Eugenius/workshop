...
typedef struct test_behaviour {
    bool wait_on_fail; /**< Wait before going to cleanup in case of
                           * test failure */
    bool wait_on_cleanup; /**< Wait before going to cleanup regardless of
                              * the test result. If both wait_on_fail and
                              * this one are set - wait will be done
                              * just once */
    bool log_stack;    /**< Enable log stack collection */

    bool log_test_fail_state;  /**< Log state/substate of the
                                       test on failure */

    bool log_all_rpc; /**< If @c true, all RPC calls should be logged
                              even if @b silent or @b silent_pass is set
                              to @c true for an RPC server. */

    bool cleanup_fd_leak_check; /**< After closing FD in
                                        CLEANUP_RPC_CLOSE(), check with
                                        rpc_fstat() that FD is really
                                        closed */

    bool cleanup_fd_close_enforce_libc; /**< If enabled, use libc-version
                                                of close() in
                                                CLEANUP_RPC_CLOSE() */
    unsigned int prologue_sleep; /**< How many sencond to sleep in prologue */

    bool fail_verdict;   /**< If enabled, log error verdict from
                                 TEST_FAIL() and CLEANUP_TEST_FAIL(),
                                 otherwise just an error */
    bool rpc_fail_verdict;    /**< If enabled, any unexpected failure of
                                      RPC call resulting in jump to cleanup
                                      will print verdict */

    bool use_chk_funcs; /**< If @c true, in RPC calls for read(), recv(),
 ...
 */
extern bool fd_not_closed_verdict;

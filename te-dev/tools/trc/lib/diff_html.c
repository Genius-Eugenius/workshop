...
"        {\n"
"            var key_checked = 'checked=\"checked\"';\n"
"        }\n"
...
"'<label for=\"' +   key + '_id\">' + val.label + '</label>'); });\n"
"\n"
"    basicChoiceContainer.find(\"input\").click(plotAccordingToChoices);\n"
"    advancedChoiceContainer.find(\"input\").click(plotAccordingToChoices);"
"\n"
"\n"
"    function plotAccordingToChoices() {\n"
"        var data = [];\n"
"\n"
"        basicChoiceContainer.find(\"input:checked\").each(function () {\n"
"            var key = $(this).attr(\"name\");\n"
"            if (key && graph_datasets[key])\n"
"                data.push(graph_datasets[key]);\n"
"        });\n"
"\n"
"        advancedChoiceContainer.find(\"input:checked\").each(function () {"
"\n"
"            var key = $(this).attr(\"name\");\n"
"            if (key && graph_datasets[key])\n"
"                data.push(graph_datasets[key]);\n"
...
{
    while (align-- != 0)
        fprintf(f, "%s", trc_diff_js_table_align);
}
                               ...
                               trc_diff_stats *stats, bool with_hashes)
{
    ...
    fprintf(f, "%s", trc_diff_js_table_start);
    ...
    TAILQ_FOREACH(set1, sets, links)
    {
        ...
        TAILQ_FOREACH(set2, sets, links)
        {
                 ...
                 status1++)
            {
                     ...
                     status2++)
                {
                         ...
                         diff++)
                    {
                        ...
                        trc_diff_stats_js_table_align(f, 6);
                        fprintf(f, "%s", trc_diff_js_table_entry_end);
                        trc_diff_stats_js_table_align(f, 5);
                        fprintf(f, trc_diff_js_table_row_end,
                                (diff < TRC_DIFF_STATUS_MAX - 1) ?
                                "," : "");
                    }
                    trc_diff_stats_js_table_align(f, 4);
                    fprintf(f, trc_diff_js_table_row_end,
                            (status2 < TRC_TEST_STATUS_MAX - 1) ?
                            "," : "");
                }
                trc_diff_stats_js_table_align(f, 3);
                fprintf(f, trc_diff_js_table_row_end, ",");
                 ...
                 summary_status++)
            {
                     ...
                     summary_component++)
                {
                    ...
                    fprintf(f, trc_diff_js_table_row_end,
                            (summary_component <
                             TRC_DIFF_SUMMARY_COMPONENT_MAX - 1) ?
                              "," : "");
                }

                trc_diff_stats_js_table_align(f, 4);
                fprintf(f, trc_diff_js_table_row_end,
                        (summary_status < TRC_DIFF_SUMMARY_STATUS_MAX - 1) ?
                        "," : "");
            }
        ...
        trc_diff_stats_js_table_align(f, 1);
        fprintf(f, trc_diff_js_table_row_end,
                (TAILQ_NEXT(set1, links) != NULL) ? "," : "");
    }

    fprintf(f, "%s", trc_diff_js_table_end);

    fprintf(f, "%s", trc_diff_js_stats_start);
    TAILQ_FOREACH(set1, sets, links)
    {
        TAILQ_FOREACH(set2, sets, links)
        {
    ...
    fprintf(f, "%s", trc_diff_js_stats_end);
}
                             ...
                             int status1, int status2, int flags)
{
    ...
    if (counters[TRC_DIFF_MATCH].counter > 0)
    {
        ...
        if (use_popups)
        {
            fprintf(f, "%s", trc_diff_stats_stats_href_close);
        }
         ...
         status1 < TRC_TEST_STATUS_MAX; status1++)
    {
             ...
             status2++)
        {
            ...
            if (status2 < TRC_TEST_UNSPECIFIED)
                fprintf(f, "%s", trc_diff_stats_table_row_mid);
        }
        fprintf(f, "%s", trc_diff_stats_table_row_end);
    }
                            ...
                            trc_diff_stats *stats, int flags)
{
    ...
    fprintf(f, "%s", trc_diff_stats_brief_table_head_end);
    ...
    fprintf(f, "%s", trc_diff_stats_brief_table_end);
                                        ...
                                        trc_diff_stats *stats, int flags)
{
    ...
    fprintf(f, "%s", trc_diff_stats_brief_incremental_table_head_start);
    if (ref_tags_count > 0)
    {
        fprintf(f, "<br/>");
        trc_diff_tags_output(f, ref_tags);
    }
    fprintf(f, "%s", trc_diff_stats_brief_incremental_table_head_end);
            ...
            if (entry->is_iter)
            {
                ...
                if (iter_data != NULL)
                {
                    TAILQ_FOREACH(iter_entry, &iter_data->runs, links)
                    {
                        ...
                        fprintf(f, "%s", trc_diff_test_result_end);
                    }
